...
describe('getMessagesSince', () => {
  ...
  it('recovers cursor from last bot reply when lastAgentTimestamp is missing', () => {
    ...
    const msgs = getMessagesSince('group@g.us', recovered!, 'Andy');
  ...
  it('caps messages to configured limit even with recovered cursor', () => {
    ...
    // All messages after the recovered cursor are returned
    const msgs = getMessagesSince('group@g.us', recovered!, 'Andy');
    // m4 (third, 00:00:04) + 30 pending messages
    expect(msgs).toHaveLength(31);
    expect(msgs[0].content).toBe('third');
    expect(msgs[30].content).toBe('pending message 30');
  });
  ...
  it('returns last N messages when no bot reply and no cursor exist', () => {
    ...
    // No cursor → sinceTimestamp = '' returns all messages
    const msgs = getMessagesSince('fresh@g.us', '', 'Andy');
    ...
    expect(messageTagCount).toBe(20);
  });
